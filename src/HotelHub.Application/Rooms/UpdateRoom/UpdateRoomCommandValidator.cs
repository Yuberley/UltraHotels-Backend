...
public class UpdateRoomCommandValidator : AbstractValidator<UpdateRoomCommand>
{
    public UpdateRoomCommandValidator()
    {
        ...
        RuleFor(x => x.Type).Must(x => RoomType.All.Any(c => c.Value == x)).WithMessage($"The room type value is invalid, only {string.Join(", ", RoomType.All.Select(c => c.Value))} are allowed");
